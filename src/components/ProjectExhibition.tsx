...
export const ProjectExhibition = ({
  ...
  const filteredProjects = projects.filter((project) => {
    ...
    if (activeCategory === 'ai') return project.category.includes('AI') || project.category.includes('LLM');
    if (activeCategory === 'fullstack') return project.category.includes('Fullstack');
    if (activeCategory === 'frontend') return project.category.includes('Frontend');
    return true;
  });
    ...
    <section id="projects" className="py-20 px-4 sm:px-6 lg:px-8 relative max-w-6xl mx-auto text-left">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-2">
          <div>
            ...
            <p className="text-sm sm:text-base text-[#94a3b8] max-w-xl mt-2 leading-relaxed">
              Explore my work and discover how I solve real-world problems.
            </p>
          </div>

          {/* Category Filter Pills */}
          <div className="flex items-center gap-1.5 p-2 rounded-xl bg-[#10131c] border border-[#1e2433] flex-wrap">
            {categories.map((cat) => (
              <button
                key={cat.id}
                onClick={() => {
                ...
                className={`px-2 py-1.5 rounded-lg text-xs font-medium transition-all ${activeCategory === cat.id && !selectedTechFilter
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {filteredProjects.map((project, index) => {
          ...
          else if (project.id === 'health-vault') layout = 'split-right';
